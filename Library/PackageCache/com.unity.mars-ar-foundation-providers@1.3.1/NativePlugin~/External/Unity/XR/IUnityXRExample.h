...
/// Sample data structure used for passing data between an XR SDK plugin and
/// the Unity runtime.
typedef struct UnityXRExampleState
{
...
/// Legacy sample data structure used for passing data between an XR SDK plugin and the Unity runtime.
/// This version does not change between versions of the plugin.
typedef struct UnityXRLegacyExampleState
{
    /// Description of theBool
    bool theBool;
} UnityXRLegacyExampleState;
...
typedef struct XR_PROVIDER UnityXRExampleProvider
{
    ...
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * UpdateState)(UnitySubsystemHandle handle, void* userData, UnityXRExampleState * state);
    ...
    /// @param state State to be updated.
    /// @return kUnitySubsystemErrorCodeFailure if state was not updated.
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * UpdateStateLegacy)(UnitySubsystemHandle handle, void* userData, UnityXRLegacyExampleState * state);
} UnityXRExampleProvider;

/// An example XR Example interface showing how to create plugin -> unity call flows with complex data, and vice-versa.
/// Also showing how to deal with binary backwards compatibility of dylibs on a potentially ever-changing interface.
UNITY_DECLARE_INTERFACE(XR_INTERFACE IUnityXRExampleInterface)
{
    ...
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * XR_REGISTER_PROVIDER(UnityXRExampleProvider) RegisterExampleProvider)(UnitySubsystemHandle handle, const UnityXRExampleProvider * provider);

    /// Example plugin -> unity call.  Dumps some state about the current XR Example handle.
    ///
    /// @param handle Handle obtained from UnityLifecycleProvider callbacks.
    ...
    /// @return Status of function execution.
    UnitySubsystemErrorCode(UNITY_INTERFACE_API * DebugLegacyState)(UnitySubsystemHandle handle, UnityXRLegacyExampleState * state);
};
UNITY_REGISTER_INTERFACE_GUID(0xAB695A1C94804266ULL, 0xBDB5A1B347AC54B8ULL, IUnityXRExampleInterface)
