...
namespace Unity.XRTools.Utils.Internal
{
    ...
    public abstract class ScriptableSettingsBase<T> : ScriptableSettingsBase where T : ScriptableObject
    {
        ...
        protected static void Save(string savePathFormat)
        {
            ...
            if (s_Instance == null)
            {
                DebugUtils.Log("Cannot save ScriptableSettings: no instance!");
                return;
            }
            ...
            if (k_HasCustomPath)
            {
                ...
                if (ValidatePath(pathAttribute.path, out path))
                {
                    ...
                    {
                        DebugUtils.LogWarning($"{k_PathExceptionMessage}\n{e}");
                        generatePath = true;
                    }
                }

                if (generatePath)
                    DebugUtils.LogWarning($"The path '{pathAttribute.path}' is invalid. Generating a path instead.");
            }
            ...
            DebugUtils.Log(string.Format("Created initial copy of settings: {0} at {1}", GetFilePath(), savePath));
#endif
        }
